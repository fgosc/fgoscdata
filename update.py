...
bg_rarity = {3: "silver", 4: "gold", 5: "gold"}

bg_url = (
    "https://raw.githubusercontent.com/atlasacademy/aa-db/master/build/assets/list/"
)
...
for n in tmp:
    ...
    for m in n.keys():
        if m.startswith("alias"):
            if n[m] != "":
                tmp_alias.append(n[m])
    if len(tmp_alias) > 0:
        name_alias[int(n["id"])] = tmp_alias
...
def compute_battle_hash(img_rgb):
    ...
    height, width = img_rgb.shape[:2]
    img = img_rgb[
        int(17 / 135 * height) : int(77 / 135 * height),
...
def compute_hash_ce_narrow(img_rgb):
    ...
    height, width = img_rgb.shape[:2]
    img = img_rgb[
        int(30 / 206 * height) : int(155 / 206 * height),
    ...
    for item in tqdm(item_list):
        ...
        if item["name"] not in name_list:
            ...
            if item["id"] in id2hash_replace:
                ...
                Image_file = search_item_file(
                    item["icon"].replace(".png", "_bordered.png"),
                    Image_dir / str(item["background"]),
                )
            ...
            ##            elif 6500 < item["id"] < 6999:
                ...
                pieces.append(tmp)
            elif item["type"] in ["eventPoint", "dice"] or item["name"] in ticket_list:
...
def make_ce_data():
    ...
    with open(CE_blacklist_file, encoding="UTF-8") as f:
        bl_ces = [s.strip() for s in f]
    with open(CE_gacha_file, encoding="UTF-8") as f:
        gacha_ces = [s.strip() for s in f]
    for ce in tqdm(ce_list):
        if ce["rarity"] <= 2:
            continue
        name = ce["name"]
        if ce["atkMax"] - ce["atkBase"] + ce["hpMax"] - ce["hpBase"] == 0 and not ce[
            "name"
        ].startswith("概念礼装EXPカード："):
            continue
        # 除外礼装は読み込まない
        ...
        star_image = cv2.imread(
            str(Misc_dir / ("star" + str(ce["rarity"]) + ".png")),
        ...
        for h in hash_narrow[0]:
            out_narrow = out_narrow + f"{h:02x}"
        tmp = {}
        tmp["id"] = ce["id"]
        tmp["type"] = "Craft Essence"
        tmp["rarity"] = ce["rarity"]
        tmp["name"] = name
        if ce["id"] in id2name_eng:
...
if __name__ == "__main__":
    ...
    with open(Drop_json_file, "w", encoding="UTF-8") as f:
        f.write(
            json.dumps(
                reward_output + ce_output + item_output + exp_output,
                ensure_ascii=False,
                indent=4,
            ),
        )
